...
def generate_calendar():
    ...
    # Generate table with weeks as columns
    output += f'<table border="1">\n<tr><th colspan="{len(calendar.monthcalendar(year, month)) + 1}"><h1>{italian_month_names[month]} {year}</h1></th></tr>\n'

    raw_calendar = calendar.monthcalendar(year, month)
    max_weeks = len(raw_calendar)

    # Transpose the raw_calendar to have weeks as columns
    for day_idx in range(7):
        output += '<tr>'
        for week_idx in range(max_weeks):
            day = raw_calendar[week_idx][day_idx]
            if day != 0:
